...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.radio_box_2 = wx.RadioBox(self, wx.ID_ANY, u"定义运算步数", choices=[u"一步", u"二步", u"三步"], majorDimension=1,
                                       style=wx.RA_SPECIFY_ROWS)
        self.radio_box_3 = wx.RadioBox(self, wx.ID_ANY, u"运算题型设置", choices=[u"求结果", u"求中间算数项"], majorDimension=1,
                                       style=wx.RA_SPECIFY_ROWS)
        ...
        self.checkbox_1 = wx.CheckBox(self, wx.ID_ANY, u"运算过程使用括号")
        self.radio_box_4 = wx.RadioBox(self, wx.ID_ANY, u"加法进位设置", choices=[u"随机进位", u"加法进位", u"没有进位"], majorDimension=1,
                                       style=wx.RA_SPECIFY_ROWS)
        self.radio_box_5 = wx.RadioBox(self, wx.ID_ANY, u"减法进位设置", choices=[u"随机退位", u"减法退位", u"没有退位"], majorDimension=1,
                                       style=wx.RA_SPECIFY_ROWS)
        self.text_ctrl_16 = wx.TextCtrl(
            self, wx.ID_ANY, "20", style=wx.TE_CENTRE)
        self.button_6 = wx.Button(self, wx.ID_ANY, u"添加数学题")

        self.button_7 = wx.Button(self, wx.ID_ANY, u"清空数学题")

        self.text_ctrl_1 = wx.TextCtrl(self, wx.ID_ANY, "此处将提示您相关操作信息！", style=wx.TE_CENTRE)
        ...
        self.button_3 = wx.Button(self, wx.ID_ANY, u"设置数学试卷保存目录")
        self.text_ctrl_4 = wx.TextCtrl(self, wx.ID_ANY, u"小学数学题")
        self.text_ctrl_5 = wx.TextCtrl(self, wx.ID_ANY, u"姓名：__________ 日期：____月____日 时间：________ 对题：____道",
                                       style=wx.TE_LEFT)
        self.button_8 = wx.Button(self, wx.ID_ANY, u"点此生成数学题文档")

        self.app_title = "HLearning小学数学试题生成器"
        self.info_tit = "您还没添加任何数学题到试卷中，请单击添加数学题按钮开始添加数学题！"  # 当前口算题卷子包含内容
    ...
    def __do_layout(self):
        ...
        sizer_14 = wx.StaticBoxSizer(wx.StaticBox(
            self, wx.ID_ANY, u"试卷标题设置"), wx.HORIZONTAL)
        sizer_13 = wx.StaticBoxSizer(wx.StaticBox(
            self, wx.ID_ANY, u"数学试卷设置"), wx.HORIZONTAL)
        sizer_12 = wx.StaticBoxSizer(wx.StaticBox(
            self, wx.ID_ANY, u"操作提示内容"), wx.HORIZONTAL)
        sizer_11 = wx.StaticBoxSizer(wx.StaticBox(
            self, wx.ID_ANY, u"添加数学题到试卷"), wx.VERTICAL)
        sizer_22 = wx.BoxSizer(wx.HORIZONTAL)
        sizer_23 = wx.BoxSizer(wx.HORIZONTAL)
        sizer_3 = wx.StaticBoxSizer(wx.StaticBox(
            self, wx.ID_ANY, u"运算详细过程设置"), wx.VERTICAL)
        ...
        sizer_2 = wx.StaticBoxSizer(wx.StaticBox(
            self, wx.ID_ANY, u"设置运算参数"), wx.HORIZONTAL)
        ...
        label_17 = wx.StaticText(self, wx.ID_ANY, u"数学题数：")
      ...
      #  sizer_1.Add(sizer_12, 0, 0, 0)

        label_2 = wx.StaticText(self, wx.ID_ANY, u"生成试卷数量：")
        sizer_13.Add(label_2, 0, 0, 0)
        sizer_13.Add(self.text_ctrl_2, 0, wx.LEFT, 8)
        label_3 = wx.StaticText(self, wx.ID_ANY, u"每列数学题个数：")
        ...
        label_4 = wx.StaticText(self, wx.ID_ANY, u"试卷标题：")
        sizer_14.Add(label_4, 0, 0, 0)
        sizer_14.Add(self.text_ctrl_4, 0, 0, 0)
        label_5 = wx.StaticText(self, wx.ID_ANY, u"试卷副标题：")
    ...
    def createPSM(self, e):
        ...
        if step == 1:
            ...
            if signum == 1:
                self.psm_info ="您选择了生成一步计算的"+ self.psm_info + "加法数学题" + str(number) + "道！"
                print(self.psm_info)
                self.text_ctrl_1.SetValue(self.psm_info)
            elif signum == 2:
                self.psm_info = "您选择了生成一步计算的" + self.psm_info + "减法数学题" + str(number) + "道！"
                self.text_ctrl_1.SetValue(self.psm_info)
            elif signum == 3:
                self.psm_info = "您选择了生成一步计算的" + self.psm_info + "乘法数学题" + str(number) + "道！"
                self.text_ctrl_1.SetValue(self.psm_info)
            elif signum == 4:
                self.psm_info = "您选择了生成一步计算的" + self.psm_info + "除法数学题" + str(number) + "道！"
                self.text_ctrl_1.SetValue(self.psm_info)
            else:
                raise Exception("题型选择错误！")

        elif step == 2:
            self.psm_info = "您选择了生成" + self.psm_info + "两步计算的数学题" + str(number) + "道！"
            self.text_ctrl_1.SetValue(self.psm_info)

        elif step == 3:
            self.psm_info = "您选择了生成" + self.psm_info + "三步计算的数学题" + str(number) + "道！"
            self.text_ctrl_1.SetValue(self.psm_info)
    ...
    def producePSM(self, e):
        ...
        if len(self.psm_type) == 0:
            print('还没有添加口算题到列表中！')  # 打印测试
            wx.MessageBox('您还未添加数学题至列表中！', '提示',
            ...
            # self.movdocx()
            wx.MessageBox('试卷生成成功，保存在'+self.config.c.get('config','docx')+'目录下，敬请查看！！', '成功提示',
                          wx.OK | wx.ICON_INFORMATION)
...
class MyDialog(wx.Dialog):
    ...
    def __do_layout(self):
        ...
        label_6 = wx.StaticText(
            self, wx.ID_ANY, u"算数项和结果是指，例如：3+7=10，3和7是算数项，10为结果。")
        ...
        label_15 = wx.StaticText(self, wx.ID_ANY, u"运 算 结果取值范围   ：")
...
class MyDialog1(wx.Dialog):
    ...
    def __do_layout(self):
        ...
        label_18 = wx.StaticText(
            self, wx.ID_ANY, u"此处为多步运算题生成运算符号选择，例如4+8-5=，\n你要做的是选择+和-号位置可以使用什么运算符号。")
